...
func (o *streamerReceiveDiamond) handleMessages(msg []byte) {
	ctx := context.Background()
	jsonData := &queue.StreamerReceiveDiamond{}
	if err := json.Unmarshal(msg, jsonData); err != nil {
		zlogger.Errorw("receiveDiamond::receiveDiamond, unmarshal msg fail", zap.Error(err))
		return
	}
	...
	// 加锁防止重复处理
	lockSign := fmt.Sprintf("lock:room_:%v", jsonData.RoomId)
	isLock, retFun := redis.TryGetDistributedLock(lockSign, lockSign, 10000, 10000)
	if !isLock {
		zlogger.Errorf("receiveDiamond tryGetDistributedLock |roomId:%v| err: failed to acquire lock", jsonData.RoomId)
		return
	}

	defer retFun()

	roomCache, err := redis.GetRoomCache(jsonData.RoomId)
	if err != nil {
		zlogger.Errorw("receiveDiamond::receiveDiamond, get room cache error", zap.Error(err))
		return
	}
	...
	if err = redis.SetRoomCache(jsonData.RoomId, roomCache); err != nil {
